...
export const generateObservationReportPDF = async (
  items,
  reportDateStr,
  explicitReportName,
) => {
  ...
  const leftLogoUrl =
    buildMediaUrl(
      template?.report_logo_url || template?.report_logo || template?.logo_url,
    ) || buildMediaUrl("/media/right_logo.png");
  const rightLogoUrl =
    buildMediaUrl(
      template?.report_logo_right_url || template?.report_logo_right,
    ) || buildMediaUrl("/media/left_logo.jpeg");
  ...
  const head = [
    ...
    [
      {
        ...
        colSpan: 3,
        rowSpan: 2,
        styles: { fillColor: whiteFill, halign: "center", valign: "middle" },
      }, // ARKADE
      {
        content: `<b>Format No.:</b> ${meta.format_no || "ADL-OH&S-F012"}`,
        colSpan: 3,
        styles: { fillColor: peachFill, halign: "left" },
      },
      {
        content: `<b>Revision No.:</b> ${meta.revision_no || "R0"}`,
        colSpan: 3,
        styles: { fillColor: peachFill, halign: "left" },
      },
      {
    ...
    [
      {
        content: `<b>Issued Date:</b> ${formattedIssuedDate}`,
        colSpan: 3,
        styles: { fillColor: peachFill, halign: "left" },
      },
      {
        content: `<b>Revision Date:</b>`,
        colSpan: 3,
        styles: { fillColor: peachFill, halign: "left" },
      },
    ],
    ...
    [
      {
        content: `<b>SAFETY & HOUSEKEEPING OBSERVATION REPORT</b>`,
        colSpan: 3,
        rowSpan: 2,
        styles: { fillColor: peachFill, halign: "center", valign: "middle" },
      },
      {
        content: `<b>Project:</b> ${projectName}`,
        colSpan: 3,
        rowSpan: 2,
        styles: { fillColor: peachFill, halign: "left", valign: "middle" },
      },
      {
        content: `<b>Date of Observation:</b> ${reportDateStr}`,
        colSpan: 3,
        styles: { fillColor: peachFill, halign: "left", valign: "middle" },
      },
    ],
    ...
    [
      {
        content: `<b>Report No.:</b> ${reportTitleText}`,
        colSpan: 3,
        styles: { fillColor: peachFill, halign: "left", valign: "middle" },
      }, // Fixed colSpan from 3 to 3, total 3+3+3+1 = 10 columns
    ],
    ...
    [
      {
        content: "",
        colSpan: 10,
        styles: { fillColor: whiteFill, minCellHeight: 8, lineWidth: 0 },
      },
    ],
    // Data Columns Row
    [
      ...
      {
        content: "PHOTOGRAPH OF UNSAFE ACT / CONDITION",
        styles: {
          fillColor: headerGreenFill,
          textColor: [0, 112, 192],
          fontStyle: "bold",
  ...
  for (let i = 0; i < sortedItems.length; i++) {
    ...
    if (
      activeSub &&
      (activeSub.final_approved_image_url || activeSub.final_approved_comments)
    ) {
      closerPhotosAndComments.push({
        url:
          activeSub.final_approved_image_url ||
          activeSub.latest_maker_photo_url ||
          activeSub.photo_url,
        comment:
          activeSub.final_approved_comments || activeSub.checker_remarks || "",
      });
      // 2. Second priority: Final selected image via is_final_image on closer_photographs
    } else if (
      item.closer_photographs &&
      Array.isArray(item.closer_photographs) &&
      item.closer_photographs.some((p) => p.is_final_image)
    ) {
      item.closer_photographs
        .filter((p) => p.is_final_image)
        .forEach((p) => {
      ...
      item.reject_photographs &&
      Array.isArray(item.reject_photographs) &&
      item.reject_photographs.some((p) => p.is_final_image)
    ) {
      item.reject_photographs
        .filter((p) => p.is_final_image)
        .forEach((p) => {
      ...
      item.closer_photographs &&
      Array.isArray(item.closer_photographs) &&
      item.closer_photographs.length > 0
    ) {
      ...
      closerPhotosAndComments.push({
        ...
        comment:
          item.closer_photograph_comments ||
          item.closer_photograph_comment ||
          item.closer_comment,
      });
      ...
      closerPhotosAndComments.push({
        url:
          activeSub.latest_maker_photo_url ||
          activeSub.photo_url ||
          activeSub.latest_checker_reject_photo_url,
        comment:
          activeSub.checker_remarks ||
          activeSub.latest_checker_reject_remarks ||
          activeSub.latest_maker_remarks ||
          activeSub.maker_remarks,
      });
    }
    ...
    for (const pc of closerPhotosAndComments) {
      const b64 = pc.url
        ? await getBase64ImageFromUrl(buildMediaUrl(pc.url))
        : null;
      closerBase64s.push({ base64: b64, comment: pc.comment || "" });
    }

    const obsPhotoUrl = item.photograph_of_unsafe_act;
    const obsPhotoBase64 = obsPhotoUrl
      ? await getBase64ImageFromUrl(buildMediaUrl(obsPhotoUrl))
      : null;
    ...
    try {
      const hazardCats =
        typeof item.hazard_categories === "string"
          ? JSON.parse(item.hazard_categories)
          : item.hazard_categories;
      const cleanHazards = (hazardCats || []).map((h) =>
        h.replace(/^\d+\.\s*/, ""),
      );
      ...
      const cleanHazards = (item.hazard_categories || "").replace(
        /^\d+\.\s*/,
        "",
      );
      if (cleanHazards) {
    ...
    if (item.risk) {
      ...
      let riskColorTag = "";
      if (item.risk === "Low Risk") riskColorTag = "<color:green>";
      else if (item.risk === "Medium Risk") riskColorTag = "<color:yellow>";
      else if (item.risk === "High Risk") riskColorTag = "<color:red>";

      hazardStr += `<b>Risk:</b>\n${riskColorTag}${item.risk}${riskColorTag ? `</color:${riskColorTag.split(":")[1]}` : ""}`;
    }
    ...
    if (closerBase64s.length > 0) {
      cellImages.push({ row: rowIndex, col: 8, items: closerBase64s });
    }
    ...
    body.push([
      ...
      { content: "", styles: { minCellHeight: 75 } },
      hazardStr,
      item.name_of_contractor || "",
      {
        content: item.target_date || "",
        styles: {
          fillColor:
            item.risk === "Low Risk"
              ? [240, 253, 244]
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      1: { cellWidth: 110, halign: "center" }, // WHAT UNSAFE ACT (was 95)
      2: { cellWidth: 70 }, // LOCATION
      3: { cellWidth: 90, minCellHeight: 85 }, // PHOTOGRAPH
      4: { cellWidth: 95 }, // HAZARD/RISK (was 80)
      5: { cellWidth: 70 }, // CONTRACTOR
      6: { cellWidth: 60 }, // TARGET DATE
      7: { cellWidth: 125, halign: "left" }, // CA/PA (was 110)
      8: { cellWidth: 90, minCellHeight: 85 }, // CLOSER PHOTOGRAPH
    },
    ...
    didDrawCell: function (data) {
      ...
      if (typeof rawText === "string" && rawText.includes("<b>")) {
        ...
        let y =
          data.cell.y +
          (data.cell.styles.cellPadding.top ||
        ...
        let lineHeight = doc.getLineHeight
          ? doc.getLineHeight()
          : data.cell.styles.fontSize * 1.15 * ptToMm;

        // If it's a body cell, we handle wrapping
        if (data.section === "body") {
          const maxWidth =
            data.cell.width - (data.cell.styles.cellPadding.left || 3) * 2;
          ...
          paragraphs.forEach((p) => {
            ...
            pText = pText.replace(/<\/?color:(green|yellow|red)>/g, "");
          ...
          y =
            data.cell.y +
            (data.cell.height - totalTextHeight) / 2 +
            data.cell.styles.fontSize * ptToMm;
          ...
          paragraphs.forEach((p) => {
            ...
            let txtColor = [0, 0, 0];
            if (pText.includes("<color:green>")) {
              txtColor = [21, 128, 61];
              pText = pText.replace(/<\/?color:green>/g, "");
            } else if (pText.includes("<color:yellow>")) {
              txtColor = [161, 98, 7];
              pText = pText.replace(/<\/?color:yellow>/g, "");
            } else if (pText.includes("<color:red>")) {
              txtColor = [185, 28, 28];
              pText = pText.replace(/<\/?color:red>/g, "");
            }
            ...
            doc.setTextColor(0, 0, 0); // Reset for next elements
          });
        } else {
      ...
      if (data.section === "head") {
        ...
        const cellImage = cellImages.find(
          (ci) => ci.row === rowIdx && ci.col === colIdx,
        );
        ...
        if (
          ...
          let currentY =
            data.cell.y + (data.cell.height - totalContentHeight) / 2;
