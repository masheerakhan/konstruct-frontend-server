...
import { CheckCircle2, List, Type, Table, LayoutTemplate, HelpCircle } from "lucide-react";

export default function HorizontalExcelPreview({
    formatMode,
    schema,
    onBack,
}) {
    ...
    const formatName = formatMode === "HORIZONTAL_TYPE_A" ? "Horizontal (Type A)" : "Grouped (Type B)";
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-emerald-100 shadow-sm overflow-hidden">
                <div className="bg-emerald-50/50 px-6 py-5 border-b border-emerald-100 flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-gray-900"> Successfully Detected</h2>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-gray-50 rounded-xl p-5 border border-gray-100">
                        ...
                        <div className="text-base font-medium text-gray-900">{formatName}</div>
                    </div>

                    <div className="bg-gray-50 rounded-xl p-5 border border-gray-100">
                        <div className="flex items-center gap-2 text-gray-500 mb-2">
                            <Table className="w-4 h-4" />
                            <span className="text-xs font-semibold uppercase tracking-wider">Columns Extracted</span>
                        </div>
                        <div className="text-base font-medium text-gray-900">{headers.length} Columns</div>
                    </div>

                    <div className="bg-gray-50 rounded-xl p-5 border border-gray-100">
                        <div className="flex items-center gap-2 text-gray-500 mb-2">
                            <List className="w-4 h-4" />
                            <span className="text-xs font-semibold uppercase tracking-wider">Detected Legend</span>
                        </div>
                        <div className="text-base font-medium text-gray-900">
                            {legend.length > 0 ? legend.join(" / ") : "None"}
                            {schema?.fallbackUsed && <span className="ml-2 text-xs text-orange-500 font-medium">(Default)</span>}
                        </div>
                    </div>
                </div>
                ...
                <div className="px-6 pb-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                        {headers.map((h, idx) => (
                            <div key={idx} className="bg-white border border-gray-200 rounded-lg p-3 flex items-start gap-3 shadow-sm hover:border-orange-200 transition-colors">
                                <div className="bg-gray-50 text-gray-400 p-1.5 rounded-md shrink-0 mt-0.5">
                                    {h.type === "multiple_choice" ? <HelpCircle className="w-4 h-4" /> : <Type className="w-4 h-4" />}
                                </div>
                                <div className="overflow-hidden">
                                    <div className="text-xs font-bold text-gray-900 truncate" title={h.text}>
                                        {h.text}
                                    </div>
