...
import { Plus, X, Trash2, Copy, Image as ImageIcon } from "lucide-react";
...
function QuestionCard({
  ...
  const updateField = (key, value) => {
    if (validationErrors?.[`question-card-${index}`])
      onClearError(`question-card-${index}`);
            ...
            {(question.referenceImageFile ||
              ...
              <div className="relative inline-block mr-2">
                <img
                  src={
                    question.referenceImageFile
                      ? URL.createObjectURL(question.referenceImageFile)
                      : question.reference_image_url || question.reference_image
                  }
                  alt="Reference"
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.files && e.target.files[0]) {
                  updateField("referenceImageFile", e.target.files[0]);
                }
              }}
            />
            <button
              ...
              className="h-8 w-8 flex items-center justify-center rounded-full text-gray-400 hover:text-orange-600 hover:bg-orange-50"
            >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  onUpdate({
                    ...question,
                    photo_required: nextVal,
                    required: nextVal,
                  });
                }}
                className={`relative inline-flex h-5 w-9 items-center rounded-full transition ${
                  isPhotoRequired ? "bg-orange-500" : "bg-gray-300"
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition ${
                    isPhotoRequired ? "translate-x-4" : "translate-x-1"
                  }`}
                />
              </button>
            </div>
