...
function getCellValue(template, column, index, projectMap) {
  ...
  if (column.id === "project") {
    ...
    const name =
      projectMap?.get(pid) ?? template.project_name ?? template.project?.name;
...
function SafetyFormats() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("Safety Checklist");
  ...
  const filteredTemplates = useMemo(() => {
    ...
    if (selectedCategory !== "All") {
      result = result.filter((t) => {
        const cat = (t.category_name ?? t.category?.name ?? "")
          .toString()
          .toLowerCase();
        if (selectedCategory === "Safety Checklist") {
          return cat.includes("checklist") || cat.includes("inspection");
        } else if (selectedCategory === "Observation") {
          return cat.includes("observation");
        }
        return true;
      });
    }
    ...
    return result.filter((t) => {
      ...
      const formatNo = (
        getFormatNo(t.report_header_meta) ??
        t.template_code ??
        t.id ??
        ""
      )
  ...
  }, [templates, searchQuery, projectMap, selectedCategory]);
  ...
  const openEditTemplate = (template) => {
    ...
    navigate(`/safetySetup/create?editTemplateId=${id}`);
  };
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            ...
                            disabled={
                              deletingId === (template.id ?? template.pk)
                            }
          ...
          <div className="h-full w-full overflow-auto pt-16 pb-4 px-3 md:px-6 lg:pl-72">
            <div className="mx-auto w-full max-w-6xl rounded-2xl bg-white shadow-2xl border border-gray-200">
              ...
              <div className="p-3 md:p-5">
                {viewLoading ? (
                  <div className="rounded-xl bg-white p-8 text-center text-muted-foreground">
                    Loading template preview...
                  </div>
                ) : viewingTemplate ? (
                  (() => {
                    if (viewingTemplate?.report_layout?.is_horizontal) {
                      return (
                        <SafetyReportTemplateHorizontal
                          schema={{
                            type:
                              viewingTemplate?.report_layout
                                ?.horizontal_format || "HORIZONTAL",
                            headers: (viewingTemplate?.questions || []).map(
                              (q) => ({
                                ...q,
                          ...
                          reportTitle={
                            viewingTemplate?.title || viewingTemplate?.name
                          }
