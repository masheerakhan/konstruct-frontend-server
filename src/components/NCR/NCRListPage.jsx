...
export default function NCRListPage({ mode = "all", role = "maker" }) {
  ...
  useEffect(() => {
    ...
    const fetchFunc = () => {
      if (mode === "my-pending")
        return getMyPending({ role, page: 1, pageSize: 50 });
      if (mode === "project-head-pending")
        return getProjectHeadPendingNCRs({ page: 1, pageSize: 50 });
      return getNCRList({ status: activeStatus, page: 1, pageSize: 50 });
    };
  ...
  const getNCRRowPath = (row) => {
    if (mode === "my-pending" || isMaker) {
      return `/ncr/${row.id}/respond`;
    }

    if (mode === "project-head-pending") {
      return `/ncr/${row.id}/sign`;
    }

    if (row.status === "maker_submitted") {
      return `/ncr/${row.id}/verify`;
    }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1
            ...
            data-testid="ncr-list-title"
          >
            {mode === "my-pending"
              ? "Your Pending NCRs"
              : mode === "project-head-pending"
                ? "NCRs Pending Your Signature"
                : "Non-Conformity Reports"}
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            {mode === "my-pending"
              ? "NCRs awaiting your action"
              : mode === "project-head-pending"
                ? "Sign formal NCR approvals"
                : "Manage and track all NCRs across projects."}
          </p>
        </div>
        {!isMaker && (
          <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-stretch auto-rows-fr">
        {loading && !summary ? (
          Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="h-full min-h-[112px] bg-white rounded-xl border border-slate-200 p-4 animate-pulse"
            >
      ...
      {/* List Section */}
      <div
        className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm transition-opacity duration-150"
      ...
      >
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          <h2 className="text-sm font-bold font-display text-slate-900">
            {activeStatus === "all"
              ? "All Records"
              : NCR_STATUSES.find((s) => s.key === activeStatus)?.label}
          </h2>
          <div className="text-xs text-slate-500 font-medium">
            {listData.length} entries found
          </div>
        </div>
            ...
            <div className="hidden sm:block overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-sm">
                  {listData.map((row) => {
                    const statusColor = NCR_STATUSES.find(
                      (s) => s.key === row.status,
                    )?.color;
                    const borderClass =
                      STATUS_BORDER_MAP[statusColor] || "border-l-transparent";
                    return (
                      <tr
                        key={row.id}
                        onClick={() => navigate(getNCRRowPath(row))}
                      ...
                      >
                        <td className="px-4 md:px-6 py-3.5 font-mono font-medium text-slate-900">
                          {row.ncr_no}
                        </td>
                        <td className="px-4 md:px-6 py-3.5 text-slate-600">
                          {row.non_conformity_related_to ||
                            row.full_location ||
                            "-"}
                        </td>

                        <td className="px-4 md:px-6 py-3.5">
                          <NCRClassificationBadge
                            classification={row.classification}
                          />
                        </td>

                        <td className="px-4 md:px-6 py-3.5 text-slate-600">
                          {row.target_date_of_compliance || "-"}
                        </td>

                        <td className="px-4 md:px-6 py-3.5">
                          <NCRStatusBadge status={row.status} />
                        </td>
                        <td className="px-4 md:px-6 py-3.5 text-slate-500">
                          {row.raised_at
                            ? new Date(row.raised_at).toLocaleDateString(
                                "en-GB",
                              )
                            : "-"}
                        </td>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center text-xs text-slate-500">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${row.classification === "major" ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"}`}
                      >
                        {row.classification}
                      </span>
                      <span>
