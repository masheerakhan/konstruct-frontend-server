...
const ReadonlyQuestionCard = ({ item, idx, pendingStatusLabel }) => {
    ...
    return (
        <div
            className={`rounded-2xl border p-4 shadow-sm ${sub?.is_resubmitted
            ...
            <div className="mb-4 flex flex-wrap gap-2">
                {(item?.options || ["YES", "NO", "NA"]).map((opt) => {
                    ...
                    return (
                        <div
                            key={opt}
                            className={`rounded-full border px-4 py-2 text-xs font-semibold transition ${selected
