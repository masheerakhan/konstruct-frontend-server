...
import { Plus, Trash2, Copy, GripVertical, ClipboardList } from "lucide-react";
...
const CreateSafetyInspection = () => {
  ...
  const [templateWorkflowConfig, setTemplateWorkflowConfig] = useState([]);
    ...
    (async () => {
      try {
        ...
        if (alive && t) {
          ...
          const savedWorkflow =
            t?.report_layout?.workflow_config &&
            Array.isArray(t.report_layout.workflow_config)
              ? t.report_layout.workflow_config
              : [];

          setTemplateWorkflowConfig(savedWorkflow);
  ...
  const handleSubmit = async () => {
    ...
    const assignments = (templateWorkflowConfig || [])
      .filter((u) => u.role && u.user_id)
      .map((u, i) => ({
        order_index: i + 1,
        role: u.role,
    ...
    if (assignments.length === 0) {
      showToast(
        "Selected format does not have Flow Config. Please update the safety setup template.",
        "error",
      );
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-4xl space-y-6 p-4 sm:p-6">
        ...
        {/* Category + Format */}
        <div className="grid grid-cols-1 gap-4 rounded-lg border bg-white p-5 sm:grid-cols-2">
          <div>
            ...
            questions.map((q) => (
              <div key={q.id} className="bg-white border rounded-lg p-5">
                <div className="mb-4 flex gap-2">
                  ...
                  <div className="grid flex-1 grid-cols-1 gap-3 sm:grid-cols-[1fr_180px]">
                    <input
                      value={q.title}
                      onChange={(e) =>
                            ...
                            <div key={idx} className="flex flex-wrap items-end gap-3">
                                <div className="flex flex-col">
                                    ...
                                    <select
                                        ...
                                        className="h-10 w-full rounded-md border px-3 sm:min-w-[140px]"
                                    >
                                        {ROLE_OPTIONS.map((r) => (
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <select
                                        ...
                                        className="h-10 w-full rounded-md border px-3 sm:min-w-[180px]"
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
          <button
            ...
            className="h-10 rounded-md border px-6 hover:bg-gray-50"
          >
            Cancel
          </button>
          <button
