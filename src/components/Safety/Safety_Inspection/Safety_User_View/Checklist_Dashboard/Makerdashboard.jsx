...
import { DynamicSafetyInput } from "./SafetyChecklistCreateView";
...
const TemplateOptionButtons = ({ options = [], value, onChange }) => {
...
export default function MakerDashboard() {
  // Helper to safely parse report_layout which might be a stringified JSON
  const getTableStyle = (template) => {
    if (!template?.report_layout) return null;
    let layout = template.report_layout;
  ...
  const [horizontalRows, setHorizontalRows] = useState([1]);
  ...
  const makerCanRectify = (c) =>
    makerInWorkflow(c) && isMakerTurn(c) && rejectedPoints(c) > 0;
    ...
    detail?.items
      ?.flatMap((item) =>
        (item.submissions || []).map((sub) => ({
          item,
          sub,
          editable: MAKER_REJECTED_STATUSES.includes(sub.status),
          highlighted: MAKER_REJECTED_STATUSES.includes(sub.status),
        })),
      )
      .filter(Boolean) ?? [];
  ...
  const fixHorizontalRows = Array.from(
    new Set(makerFixItems.map((x) => Number(x.sub?.row_index || 1))),
  ).sort((a, b) => a - b);
  const isFixHorizontal =
    fixHorizontalRows.length > 1 ||
    makerFixItems.some((x) => (x.item?.submissions || []).length > 1);

  const toggleFixRow = (rowIdx) =>
    setExpandedRows((prev) => ({
      ...prev,
      [`fix_${rowIdx}`]: !prev[`fix_${rowIdx}`],
    }));

  const renderFixAccordionWrapper = (rowIdx, arrayIdx, itemsRenderer) => {
    const isExpanded = expandedRows[`fix_${rowIdx}`] !== false; // Default expanded
    return (
      <div
        key={rowIdx}
  ...
  const renderFixItemList = (itemsList, renderer) => {
    if (isFixHorizontal) {
      return (
        <div className="space-y-4">
          {fixHorizontalRows.map((rowIdx, arrayIdx) => {
            const rowItems = itemsList.filter(
              (x) => Number(x.sub?.row_index || 1) === rowIdx,
            );
            return renderFixAccordionWrapper(rowIdx, arrayIdx, () =>
  ...
  const handleSubmitAllFixes = async () => {
    ...
    const submissions = makerFixItems
    ...
    if (!submissions.length) {
      showToast("No rejected questions to submit.", "error");
      return;
    }

    const missingAnswer = submissions.find((s) => !s.answer);

    if (missingAnswer) {
      showToast("Please answer all rejected questions.", "error");
      return;
    }
  ...
  const handleCreateAndSubmitChecklist = async () => {
    ...
    if (unansweredIndex !== -1) {
      const rowStr =
        unansweredRowIdx !== -1
          ? ` on Row ${horizontalRows.indexOf(unansweredRowIdx) + 1}`
          : "";
      showToast(
        `Please answer Question ${unansweredIndex + 1}${rowStr}`,
        "error",
      );
      return;
    }

    const missingRequiredPhoto = questions.find((q) => {
      if (getTableStyle(selectedTemplate) === "horizontal") {
        return horizontalRows.some(
          (rowIdx) => q.photo_required && !createMedia[`${q.id}_${rowIdx}`],
        );
      }
      return q.photo_required && !createMedia[q.id];
    });
    ...
    const answersPayload =
      getTableStyle(selectedTemplate) === "horizontal"
        ? horizontalRows.flatMap((rowIdx) =>
            questions.map((q) => ({
              template_question_id: q.id,
              row_index: rowIdx,
              answer: createAnswers[`${q.id}_${rowIdx}`],
              maker_remarks: createRemarks[`${q.id}_${rowIdx}`] || "",
            })),
          )
        : questions.map((q) => ({
            template_question_id: q.id,
    ...
    if (getTableStyle(selectedTemplate) === "horizontal") {
      horizontalRows.forEach((rowIdx) => {
        questions.forEach((q) => {
          const file = createMedia[`${q.id}_${rowIdx}`];
          if (file) {
            formData.append(`maker_media_${q.id}_${rowIdx}`, file);
          }
        });
      });
    } else {
                ...
                {!templateLoading && selectedTemplate && (
                  <>
                    <div className="mb-4 rounded-xl border bg-card p-5 shadow-sm">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {(selectedTemplate.questions || []).length} question(s)
                      </p>
                    </div>

                    {selectedTemplate.instruction_image && (
                                            ...
                                            <div className="space-y-3">
                                              ...
                                              (q.type !== "short_answer" &&
                                                ...
                                                <textarea
                                                  ...
                                                  className="w-full resize-y rounded-lg border border-border bg-white px-3 py-2 text-sm text-foreground focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100"
                                                  placeholder="Enter your answer..."
                                                />
                                              )}
                                              ...
                                              <div>
                                                ...
                                                <textarea
                                                  ...
                                                  className="w-full resize-y rounded-lg border border-border bg-white px-3 py-2 text-sm text-foreground placeholder-muted-foreground focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100"
                                                  placeholder="Add a remark if any..."
                                                />
                                              </div>

                                              <div className="pt-2">
                                                <PhotoUploadArea
                                                  id={`create-media-${keyId}`}
                                                  previewBase64={
                                                    createMedia[keyId]
                                                  }
                                                  label={
                  ...
                  <div className="space-y-4">
                    {renderFixItemList(
                      ...
                      ({ item, sub, editable, highlighted }, idx) => {
                        ...
                        const inputType =
                          item.input_type || item.type || "multiple_choice";
                            ...
                            <div className="space-y-3">
                              ...
                              (inputType !== "short_answer" &&
                                ...
                                <textarea
                                  ...
                                  className={`w-full resize-y rounded-lg border border-border bg-white px-3 py-2 text-sm text-foreground focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100 ${!editable ? "cursor-not-allowed opacity-60 bg-muted/30" : ""}`}
                                  placeholder="Enter your answer..."
                                />
                              )}
