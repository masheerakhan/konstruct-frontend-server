...
export default function ReviewSafetyObservation({ detail, onBack, onSuccess }) {
    ...
    const handleApprove = () => {
        const allImages = [
            ...(detail.closer_photographs || []).map(p => ({ ...p, type: 'closer' })),
            ...(detail.reject_photographs || []).map(p => ({ ...p, type: 'reject' }))
        ];
        ...
        try {
            ...
            if (signatureAction === "approve") {
                ...
                if (selectedFinalComments.length > 0) {
                    payload.append("final_approved_comments", JSON.stringify(selectedFinalComments));
                }
                ...
                if (Object.keys(photoComments).length > 0) {
                    payload.append("checker_photo_comments", JSON.stringify(photoComments));
                }
            }
        ...
        <div className="mx-auto max-w-5xl pb-12">
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">Review Safety Observation #{detail.id}</h1>
                        <p className="text-sm text-slate-500">Please review the rectification submitted by the Maker.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="rounded-xl border bg-white p-6 shadow-sm">
                <div className="space-y-8">
                    ...
                    <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
                        ...
                        <h3 className="font-bold text-slate-600 mb-4">9. CLOSER PHOTOGRAPH</h3>
                        
                        {closerPhotographs.length === 0 ? (
                            <p className="text-sm text-slate-500 italic">No closer photograph attached.</p>
                        ) : (
                    ...
                    <div className="mt-8 pt-4 border-t border-slate-100">
                        {actionType === "reject" ? (
                            <div className="rounded-xl border border-red-200 bg-red-50 p-4 mb-4">
                                <h4 className="font-semibold text-red-700 mb-2">Provide Reason for Rejection</h4>
                                <textarea
                                ...
                                <div className="mt-4 border-t border-red-100 pt-4">
                                    <h4 className="font-semibold text-red-700 mb-2">Attach Photos (Optional)</h4>
                                    <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-red-200 bg-white py-6 transition-colors hover:bg-red-50/50">
                                        <UploadCloud className="mb-2 h-8 w-8 text-red-400" />
                                        <p className="mb-1 text-sm text-slate-600">
                                            <label className="cursor-pointer font-semibold text-red-600 hover:text-red-700">
                                    ...
                                    {rejectPhotos.length > 0 && (
                                        <div className="mt-4 space-y-3">
                                            {rejectPhotos.map((photo, idx) => (
                                                <div key={idx} className="rounded-lg border border-red-200 bg-white p-3 shadow-sm flex flex-col gap-2">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-3">
                                                            <img
                                                            ...
                                                            />
                                                            <span className="text-sm font-medium text-slate-700 truncate max-w-[150px]">{photo.name}</span>
                                                        </div>
                                                        <button
                                                            type="button"
                                                            onClick={() => setRejectPhotos(prev => prev.filter((_, i) => i !== idx))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-md animate-in zoom-in-95 rounded-2xl bg-white p-6 shadow-2xl">
                        <h3 className="mb-4 text-xl font-bold text-slate-800">
                            Sign to {signatureAction === "approve" ? "Approve" : "Reject"} Observation
                        </h3>
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    fetch(sigData)
                                        .then(res => res.blob())
                                        .then(blob => {
                                            const file = new File([blob], `checker-signature-${Date.now()}.png`, { type: "image/png" });
                                            setIsSignatureModalOpen(false);
                                            confirmSubmit(file);
                                        });
                                }}
                                ...
                                disabled={submitting}
                            >
                                {submitting ? "Submitting..." : signatureAction === "approve" ? "Approve" : "Confirm Rejection"}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
                    <div className="flex w-full max-w-2xl animate-in zoom-in-95 flex-col overflow-hidden rounded-2xl bg-white p-6 shadow-2xl">
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="text-xl font-bold text-slate-800">Select Final Image</h3>
                            <button 
                        ...
                        <div className="mb-6 max-h-[50vh] overflow-y-auto pr-2">
                            <p className="text-sm text-slate-500 mb-6">
                                Please select the final image and/or comment that best represents the resolved state of this observation. You must select at least one image or comment.
                            </p>
                            
                            <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
                                {availableFinalImages.map((img) => (
                                    <div key={`${img.type}-${img.id}`} className="flex flex-col gap-2">
                                        <div 
                                            ...
                                            className={`group relative cursor-pointer overflow-hidden rounded-xl border-2 transition-all duration-200 ${
                                                selectedFinalImage?.id === img.id && selectedFinalImage?.type === img.type
                                        ...
                                        >
                                            <div className="aspect-square bg-slate-50">
                                                <img 
                                                    src={typeof img.image === 'string' && img.image.startsWith('http') ? img.image : `/users-api${img.image}`} 
                                            ...
                                            <div className="absolute bottom-0 w-full bg-slate-900/70 px-3 py-2 backdrop-blur-sm">
                                                <span className="text-xs font-semibold text-white/95">
                                                    {img.type === 'reject' ? 'Rejected Photo' : 'Maker Photo'}
                                                </span>
                                            </div>
                                        </div>
                                        {img.comment && (
                                            <label className={`flex items-start gap-2 p-2 rounded-lg border cursor-pointer transition-colors ${selectedFinalComments.includes(img.comment) ? 'border-orange-400 bg-orange-50 hover:bg-orange-100/80' : 'border-slate-200 bg-slate-50 hover:bg-slate-100'}`}>
                                                <input 
                                                    type="checkbox" 
                                                    ...
                                                    onChange={(e) => {
                                                        if (e.target.checked) {
                                                            setSelectedFinalComments(prev => [...prev, img.comment]);
                                                        } else {
                                                            setSelectedFinalComments(prev => prev.filter(c => c !== img.comment));
                                                        }
                                                    }}
                                                />
                                                <span className="text-xs text-slate-700 line-clamp-3" title={img.comment}>
                                                    {img.comment}
                                                </span>
                                            </label>
                                        )}
                        ...
                        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                            ...
                            <button
                                ...
                                disabled={!selectedFinalImage && selectedFinalComments.length === 0}
