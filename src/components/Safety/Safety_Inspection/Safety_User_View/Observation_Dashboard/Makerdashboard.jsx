...
import { ArrowLeft, Clock, ClipboardCheck, FolderOpen, Eye, Wrench, ClipboardList, CircleAlert, Plus, CheckCircle, Download, TriangleAlert, X as XIcon, Send, Upload, Activity } from "lucide-react";
    ...
    return list.filter((item) => {
        ...
        if (filters.status !== "all") {
            if (filters.status === "open") {
                if (isApprovedChecklist(item) || isRejectedChecklist(item)) return false;
            } else if (filters.status === "closed") {
                if (!isApprovedChecklist(item) && !isRejectedChecklist(item)) return false;
            } else if (filters.status === "rectification") {
                if (item.completed_by_maker !== false || item.rejected_by_checker !== true) return false;
            } else if (filters.status === "re rectification") {
                if (item.completed_by_maker !== true || item.approved_by_checker !== false || item.rejected_by_checker === true) return false;
...
export default function MakerDashboard() {
    ...
    const fetchList = async () => {
        ...
        try {
            ...
            if (isObservations && orgId) params.org_id = orgId;
    ...
    const loadCreateTemplates = async () => {
        ...
        try {
            ...
            const activeTemplates = list.filter(t => {
                ...
                const firstRole = firstWorkflowStep ? String(firstWorkflowStep.role).toUpperCase() : "";
    ...
    const makerPendingReview = filteredChecklists.filter(
        (c) => {
            if (isObservations) return statusOf(c) === "pending_checker";
            return makerInWorkflow(c) &&
                   !isFinalChecklist(c) &&
    ...
    const makerPendingRectification = filteredChecklists.filter(
        (c) => {
            if (isObservations) return statusOf(c) === "pending_maker";
            return makerInWorkflow(c) &&
                   !isFinalChecklist(c) &&
    ...
    // Completed list.
    const makerCompletedRaw = filteredChecklists.filter(
        (c) => {
    ...
    const isMakerFirstStep = initialCreateWorkflowStep && ["MAKER", "INITIALIZER"].includes(String(initialCreateWorkflowStep.role).toUpperCase());
    const createAssignedQuestionsIndices = isMakerFirstStep ? (initialCreateWorkflowStep?.assigned_questions || []) : [];
    ...
    const openMakerFix = async (cl) => {
        ...
        try {
            ...
            showToast(err?.response?.data?.detail || "Failed to load details", "error");
        } finally {
        ...
        if (isObservations) {
            ...
            if (reportGroup) {
                ...
                try {
                    ...
                    const reportOrgId = firstItem.org_id || orgId;
                    const reportProjectId = firstItem.project_id || projectId;
    ...
    const checkerApprovedCount = filteredChecklists.filter(c => isApprovedChecklist(c)).length;
    const checkerRejectedCount = filteredChecklists.filter(c => isRejectedChecklist(c)).length;
    ...
    const onTimeClosuresCount = filteredChecklists.filter(c => {
        ...
        const mapping = {};
        const sortedDates = Array.from(new Set(checklists.map(c => {
            if (!c.created_at) return null;
            return new Date(c.created_at).toLocaleDateString("en-GB", { day: '2-digit', month: '2-digit', year: 'numeric' });
    ...
    const groupedReports = React.useMemo(() => {
        const groups = {};
        const itemsToGroup = isObservations ? filteredChecklists : makerCompletedChecklists;
        ...
        const sortedDates = Object.values(groups).sort((a, b) => b.dateObj - a.dateObj).map(g => g.date);
        ...
        return reports;
    }, [filteredChecklists, makerCompletedChecklists, isObservations, reportNameByDate]);
                                    ...
                                    <>
                                        ...
                                        {!isMakerFirstStep && (
                                            <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
                                                The first step of this checklist workflow is assigned to another role (e.g. Checker). Please complete the Inspection Details above and click "Submit Checklist" to initiate the process.
                                            </div>
                                        )}
                                                    ...
                                                    <div className="space-y-3">
                                                        ...
                                                        {editable ? (
                                                            ...
                                                            <div className="flex flex-col gap-1.5">
                                                                <RemarksBubble label="Submission Answer" text={sub.answer || makerAnswers[sub.id] || "No Answer"} />
                                                            </div>
                                                        )}
                            ...
                            <section>
                                    ...
                                    {(isObservations ? [
                                        {
                                            ...
                                            icon: <ClipboardList className="h-3.5 w-3.5 text-purple-600" />,
                                ...
                                <div className="mb-6">
                                    <div className="mb-2 flex items-center gap-2">
                                        {isObservations ? <Eye className="h-4 w-4 text-orange-500" /> : <Clock className="h-4 w-4 text-yellow-500" />}
                                        <h2 className={`text-sm font-semibold sm:text-base ${isObservations ? 'text-orange-500' : 'text-yellow-500'}`}>
                                ...
                                <div className="mb-6">
                                    <div className="mb-2 flex items-center gap-2">
                                        {isObservations ? <Clock className="h-4 w-4 text-yellow-500" /> : <Send className="h-4 w-4 text-orange-500" />}
                                        <h2 className={`text-sm font-semibold sm:text-base ${isObservations ? 'text-yellow-500' : 'text-orange-500'}`}>
                                            Pending for Rectification
                                        </h2>
                                        <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                                ...
                                <div className="grid grid-cols-1 gap-6 md:grid-cols-2 mb-6">
                                    ...
                                    <div>
                                        <div className="mb-2 flex items-center gap-2">
                                            ...
                                            <h2 className="text-sm font-semibold text-foreground sm:text-base" style={{ color: "hsl(145, 65%, 42%)" }}>{isObservations ? "Closed Observations" : "Completed Checklists"}</h2>
