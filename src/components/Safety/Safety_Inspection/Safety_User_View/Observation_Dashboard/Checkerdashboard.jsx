  ...
  return (list || []).filter((item) => {
    ...
    if (isObservations) {
      ...
      if (filters.wing !== "all") {
        ...
        const locComb = String(
          item.location_combined || item.location || "",
        ).toLowerCase();
...
export default function CheckerDashboard() {
  ...
  const [isSubmitSignatureModalOpen, setIsSubmitSignatureModalOpen] =
    useState(false);
  ...
  const checkerAssigned = filteredChecklists.filter((c) => {
    if (isObservations) {
      ...
      if (isPendingChecker) {
        ...
        return (
          !c.final_checker_id || String(c.final_checker_id) === String(userId)
        );
      }
      return false;
    }
  ...
  const checkerPending = filteredChecklists.filter((c) => {
    if (isObservations) return statusOf(c) === "pending_maker";
    return (
  ...
  const checkerCompleted = filteredChecklists.filter((c) =>
    isFinalChecklist(c),
  );
  ...
  const openCheckerObservation = async (item) => {
    ...
    try {
      ...
      showToast(
        err?.response?.data?.detail || "Failed to load observation",
        "error",
      );
    } finally {
      setDetailLoading(false);
  ...
  const handleSubmitChecklist = async () => {
    ...
    const currentWA = detail.workflow_assignments?.find(
      (wa) => wa.order_index === currentStep,
    );
    const assignedQuestions = currentWA?.assigned_questions || [];

    checkerReviewItems.forEach(({ item, sub }, idx) => {
      const isAssignedHere =
        assignedQuestions.length === 0 || assignedQuestions.includes(idx);
            ...
            {view === "inspection" && detail && (
              <section>
                ...
                <div className="space-y-4">
                  {checkerReviewItems.map(({ item, sub }, idx) => {
                    ...
                    const assignedQuestions =
                      currentWA?.assigned_questions || [];
                    const isAssignedHere =
                      assignedQuestions.length === 0 ||
                      assignedQuestions.includes(idx);
                ...
                {checkerReviewItems.some(({ sub }, idx) => {
                  ...
                  return (
                    isAssignedHere && (!hasAnswer || sub.status === "created")
                  );
                }) ? (
                ...
                <section>
                  ...
                  <div className="mb-6">
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-semibold sm:text-base text-yellow-600">
                        {isObservations
                  ...
                  <div className="mb-6">
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-semibold sm:text-base text-orange-600">
                        {isObservations
                          ...
                          >
                            ...
                            <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  isObservations
                                    ? openCheckerObservation(item)
                                    : openAssignedInspection(item)
                                }
                              ...
                              >
                                Review{" "}
                                {isObservations ? "Observation" : "Checklist"}
                              </button>
                            </div>
                          </div>
                        ))
