            ...
            {normalizedOptions.map((opt) => {
                    ...
                    normalizedLabel === "yes"
                        ...
                        : normalizedLabel === "no"
                            ...
                            : normalizedLabel === "n/a" || normalizedLabel === "na"
                                ? {
                                    active: "border-yellow-500 bg-yellow-500 text-white shadow-sm",
                ...
                return (
                    <button
                        ...
                        className={`rounded-lg border px-4 py-2 text-sm font-semibold transition ${active
                            ? toneClasses.active
                            : toneClasses.idle
                            }`}
                    >
...
export const DynamicSafetyInput = ({ item, value, secondaryValue, onChange, onSecondaryChange }) => {
    const inputType = item.input_type || item.type || "multiple_choice";
    
    // Normalize options
    const options = Array.isArray(item.options) && item.options.length ? item.options : ["Yes", "No", "N/A"];
    const secondaryOptions = Array.isArray(item.secondary_options) && item.secondary_options.length ? item.secondary_options : [];
    ...
    const renderPrimaryInput = () => {
        switch (inputType) {
            ...
            case "date":
                return (
                    <input
                        ...
                        value={value || ""}
                        onChange={(e) => onChange(e.target.value)}
                        className="w-full md:w-auto rounded-lg border-gray-300 p-2.5 text-sm shadow-sm focus:border-orange-500 focus:ring-orange-500"
                    />
                );
            case "dropdown":
                return (
                    <select
                        value={value || ""}
                        onChange={(e) => onChange(e.target.value)}
                        className="w-full rounded-lg border-gray-300 p-2.5 text-sm shadow-sm focus:border-orange-500 focus:ring-orange-500 bg-white"
                    >
            ...
            case "checkbox":
                const selectedList = value ? (typeof value === "string" ? value.split(",").map(s => s.trim()) : value) : [];
                return (
            ...
            default:
                if (inputType === "file_upload") {
                    // file_upload is handled separately via the photo upload UI, but we can still show a placeholder or let photo upload area be the main thing
                    return (
                        <div className="text-sm text-gray-500 italic px-2 py-1 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                            Please use the photo attachment below.
                        </div>
                    );
                }
                return (
    ...
    const renderSecondaryInput = () => {
        if (!item.has_secondary || !item.secondary_type) return null;
        
        return (
            <div className="mt-3 pl-4 border-l-2 border-orange-200">
                <p className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Enter Answer</p>
                {(() => {
                        ...
                        case "short_answer":
                            return (
                                <input
                                    ...
                                    className="w-full rounded-lg border-gray-300 p-2 text-sm shadow-sm focus:border-orange-500 focus:ring-orange-500 bg-gray-50"
...
const getReportNumberPreview = (template) => {
    ...
    const prefix = String(
        config.prefix ||
        legacyMeta.inspection_report_prefix ||
        ""
    )
        .trim()
        .replace(/-+$/, "");
    ...
    return (
        <div className="mb-4 rounded-xl border bg-card p-5 shadow-sm">
            <div className="mb-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                    Complete the inspection details below before answering the checklist questions. Required fields marked with * must be filled and will be included in the generated report.
                </p>
            </div>
...
// MakerDashboard
// ─────────────────────────────────────────────
export default function SafetyChecklistCreateView({ onBack, allowedFirstRoles = ["MAKER", "INITIALIZER"] }) {
    ...
    const [view, setView] = useState("create"); // "dashboard" | "maker_fix"
    ...
    const [pendingCreateSubmitFormData, setPendingCreateSubmitFormData] = useState(null);
    ...
    const currentUserType = String(currentUserProfile?.user_type || "").toUpperCase();
    ...
    const loadCreateTemplates = async () => {
        ...
        try {
            ...
            const categories = Array.isArray(catRaw) ? catRaw : catRaw?.results ?? [];

            const safetyCategory = categories.find((c) => {
                const name = String(c.name || "").trim().toLowerCase();
                if (isObservations) {
                    return name.includes("observation") || name.includes("housekeeping");
                }
            ...
            const activeTemplates = list.filter(t => {
                ...
                const catName = String(t.category?.name || t.category_name || "").toLowerCase();
                const isObsTemplate = catName.includes("observation") || catName.includes("housekeeping");
                
                if (isObservations && !isObsTemplate) return false;
                if (!isObservations && isObsTemplate) return false;

                // Restrict based on first workflow step
                const sortedWorkflowConfig = t.workflow_config?.slice()?.sort((a,b) => a.order_index - b.order_index) || [];
                const firstWorkflowStep = sortedWorkflowConfig[0];
                const firstRole = firstWorkflowStep ? String(firstWorkflowStep.role).toUpperCase().trim() : "";

                // Only show templates where the current user's role is the first step.
                // admin defines who step 1 is — only that role sees the template here.
                if (firstRole && !allowedFirstRoles.includes(firstRole)) {
                    return false;
                }
    ...
    useEffect(() => {
        ...
        setReportMeta((previous) => ({
            ...previous,
            name_of_contractor:
                previous.name_of_contractor || externalContractorName,
        }));
    ...
    const backFromCreate = () => {
        ...
        setReportMeta({});
        setContractors([]);
        if (onBack) onBack(); else setView("dashboard");
    };
    ...
    const isMakerTurn = (c) =>
        ...
        (!c.current_assignee_id || Number(c.current_assignee_id) === Number(userId));
    ...
    // So use raw checklists for pending/review/rectification.
    const makerPendingReview = checklists.filter(
        (c) =>
            makerInWorkflow(c) &&
            !isFinalChecklist(c) &&
    ...
    const makerPendingRectification = checklists.filter(
        (c) =>
            makerInWorkflow(c) &&
            !isFinalChecklist(c) &&
            statusOf(c) === "in_progress" &&
    ...
    // Raw completed list.
    // Filters will apply only on this completed list.
    const makerCompletedRaw = checklists.filter(
        (c) => makerInWorkflow(c) && isFinalChecklist(c)
    );

    // Filter options should also come from completed checklists only.
    const contractorOptions = Array.from(
        new Set(makerCompletedRaw.map(getChecklistContractor).filter(Boolean))
    ).sort();

    const typeOptions = Array.from(
        new Set(makerCompletedRaw.map(getChecklistType).filter(Boolean))
    ).sort();

    // Final completed list after date/contractor/status/type filter.
    const makerCompletedChecklists = applyChecklistFilters(
        makerCompletedRaw,
        filters
    );

    // These counters are affected by filters because they belong to completed bucket.
    const makerApprovedCount = makerCompletedChecklists.filter(isApprovedChecklist).length;

    const makerRejectedCount = makerCompletedChecklists.filter(isRejectedChecklist).length;
    ...
    const makerFixItems =
        detail?.items
            ?.map((item, idx) => {
                ...
                const isAssigned = !hasAssignedQuestions || assignedQuestions.includes(idx);

                if (!sub) {
                    sub = { id: `mock-${item.id}`, status: "created", checklist_item: item.id };
                }

                const editable = isAssigned && (
                    sub.status === "created" || 
    ...
    const sortedWorkflowConfig = selectedTemplate?.workflow_config?.slice()?.sort((a,b) => a.order_index - b.order_index) || [];
    const initialCreateWorkflowStep = sortedWorkflowConfig[0];
    // isCreatorFirstStep: true if the first workflow step role matches the current user's allowed roles.
    // Controls which questions are shown and answered by the initiator.
    const isCreatorFirstStep = initialCreateWorkflowStep && allowedFirstRoles.includes(String(initialCreateWorkflowStep.role).toUpperCase().trim());
    const createAssignedQuestionsIndices = isCreatorFirstStep ? (initialCreateWorkflowStep?.assigned_questions || []) : [];
    const isCreateAssigned = (idx) => {
        if (!isCreatorFirstStep) return false;
        return createAssignedQuestionsIndices.length === 0 || createAssignedQuestionsIndices.includes(idx);
    };
    ...
    const handleSubmitAllFixes = async () => {
        ...
        makerFixItems
            ...
            .forEach(({ sub, item, isMock }) => {
                ...
                const ansData = makerAnswers[ansKey] || makerAnswers[sub.id];
                const answer = typeof ansData === "object" ? ansData?.answer || "" : ansData || "";
                const secondaryAnswer = typeof ansData === "object" ? ansData?.secondary_answer || "" : "";
                
                if (isMock) {
                    answers[item.id] = { answer: answer, secondary_answer: secondaryAnswer, type: item.input_type || item.type || "multiple_choice" };
                    ...
                    submissions.push({
                        ...
                        type: item.input_type || item.type || "multiple_choice"
                    });
                }
            });

        const hasMissingAnswer = 
            submissions.some(s => s.type !== "file_upload" && !s.answer) || 
            Object.values(answers).some(a => a.type !== "file_upload" && (typeof a === "object" ? !a.answer : !a));
    ...
    const handleSubmitWithMakerSignature = async () => {
        ...
        if (pendingSubmitPayload?.answers) {
            formData.append(
                "answers",
                JSON.stringify(pendingSubmitPayload.answers)
            );
        }
    ...
    const handleCreateAndSubmitChecklist = async () => {
        ...
        const editableHeaderFields = getTemplateHeaderFields(selectedTemplate).filter(
            isEditableMakerField
        );
        ...
        const assignedQuestionsToAnswer = questions.filter((q, idx) => isCreateAssigned(idx));

        const unansweredIndex = assignedQuestionsToAnswer.findIndex(
            (q) => {
                const type = q.input_type || q.type || "multiple_choice";
                if (type === "file_upload") return false;
                const ansObj = createAnswers[q.id];
                const ansText = typeof ansObj === 'object' && ansObj !== null ? ansObj.answer : ansObj;
                return !String(ansText || "").trim();
            }
        );
        ...
        if (unansweredIndex !== -1) {
            ...
            showToast(
                `Please answer Question: ${unanswered.text}`,
                "error"
            );
            return;
        }
        ...
        formData.append(
            "report_meta",
            JSON.stringify(cleanReportMeta)
        );
        ...
        formData.append(
            ...
            JSON.stringify(
                questions.map((q) => {
                    const ansData = createAnswers[q.id];
                    const answer = typeof ansData === "object" ? ansData?.answer || "" : ansData || "";
                    const secondaryAnswer = typeof ansData === "object" ? ansData?.secondary_answer || "" : "";
                    return {
                        ...
                        {view === "create" && (
                            <section>
                                ...
                                <div className="mb-6 rounded-xl border bg-card p-5 shadow-sm">
                                    ...
                                    >
                                        <option value="">
                                            {templatesLoading ? "Loading checklists..." : "Select checklist"}
                                        </option>
                                    ...
                                    {!templatesLoading && templates.length === 0 && (
                                        <p className="mt-2 text-xs text-red-500">
                                            No active Safety Checklist formats found for this project.
                                        </p>
                                    )}
                                ...
                                {!templateLoading && selectedTemplate && (
                                    <>
                                        <div className="mb-4 rounded-xl border bg-card p-5 shadow-sm">
                                            ...
                                            <p className="mt-1 text-xs text-muted-foreground">
                                                {(selectedTemplate.questions || []).filter((q, idx) => isCreateAssigned(idx)).length} question(s)
                                            </p>
                                        </div>
                                        ...
                                        {!isCreatorFirstStep && (
                                            <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
                                                The first step of this checklist workflow is assigned to another role. Please complete the Inspection Details above and click "Submit Checklist" to initiate the process.
                                            </div>
                                        )}
                                            ...
                                            {(selectedTemplate.questions || []).map((q, idx) => {
                                                ...
                                                >
                                                    ...
                                                    <div className="space-y-3">
                                                        <DynamicSafetyInput
                                                            item={q}
                                                            value={createAnswers[q.id]?.answer ?? (typeof createAnswers[q.id] === 'string' ? createAnswers[q.id] : "")}
                                                            secondaryValue={createAnswers[q.id]?.secondary_answer ?? ""}
                                                            onChange={(value) =>
                                                                setCreateAnswers((prev) => ({
                                                                    ...prev,
                                                                    [q.id]: { ...prev[q.id], answer: value },
                                                                }))
                                                            }
                                                            onSecondaryChange={(value) =>
                                                                setCreateAnswers((prev) => ({
                                                                    ...prev,
                                                                    [q.id]: { ...prev[q.id], secondary_answer: value },
                                                                }))
                                                            }
                                                        />
                        ...
                        {view === "maker_fix" && detail && (
                            <section>
                                ...
                                {makerEditableItems.length === 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        {makerFixItems.map(({ item, sub, editable, highlighted }, idx) => {
                                            ...
                                            return (
                                                <div
                                                    key={item.id}
                                                    className={`rounded-xl border p-5 shadow-sm transition-shadow hover:shadow-md ${highlighted
                                                ...
                                                >
                                                    ...
                                                    <div className="space-y-3">
                                                        ...
                                                        {editable ? (
                                                            <DynamicSafetyInput
                                                                item={item}
                                                                value={makerAnswers[sub.id]?.answer ?? (typeof makerAnswers[sub.id] === 'string' ? makerAnswers[sub.id] : "")}
                                                                secondaryValue={makerAnswers[sub.id]?.secondary_answer ?? ""}
                                                                onChange={(value) => {
                                                                    setMakerAnswers((prev) => ({
                                                                        ...prev,
                                                                        [sub.id]: { ...prev[sub.id], answer: value },
                                                                    }));
                                                                }}
                                                                onSecondaryChange={(value) => {
                                                                    setMakerAnswers((prev) => ({
                                                                        ...prev,
                                                                        [sub.id]: { ...prev[sub.id], secondary_answer: value },
                                                                    }));
                                                                }}
                                                            />
                                                        ) : (
                                                            <div className="flex flex-col gap-1.5">
                                                                <RemarksBubble label="Submission Answer" text={sub.answer || (typeof makerAnswers[sub.id] === 'object' ? makerAnswers[sub.id]?.answer : makerAnswers[sub.id]) || "No Answer"} />
                                                                {(sub.secondary_answer || (typeof makerAnswers[sub.id] === 'object' ? makerAnswers[sub.id]?.secondary_answer : null)) && (
                                                                    <RemarksBubble label="Additional Details" text={sub.secondary_answer || (typeof makerAnswers[sub.id] === 'object' ? makerAnswers[sub.id]?.secondary_answer : null)} />
                                                                )}
                                                            </div>
                                                        )}
                                                        ...
                                                        <div>
                                                            ...
                                                            <textarea
                                                                disabled={!editable}
                                                                className={`w-full rounded-lg border border-border bg-muted/30 p-3 text-sm ${!editable ? "cursor-not-allowed opacity-60" : ""
                                                                    }`}
                            ...
                            <section>
                                ...
                                <div className="mb-6">
                                    <div className="mb-2 flex items-center gap-2">
                                        ...
                                        <h2 className="text-sm text-yellow-500 font-semibold text-foreground sm:text-base">
                                            Pending for Review
                                        </h2>
                                    ...
                                    <div className={`rounded-xl border bg-card shadow-sm ${isObservations ? "max-h-[350px] overflow-y-auto custom-scrollbar" : ""}`}>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                setReadonlyPendingStatusLabel("Pending for Review");
                                                            }}
                                ...
                                <div className="mb-6">
                                    <div className="mb-2 flex items-center gap-2">
                                        ...
                                        <h2 className="text-sm text-orange-500 font-semibold text-foreground sm:text-base">
                                            Pending for Rectification
                                        </h2>
                                        <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                                                            <span
                                                                className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${canRectify
                                                                    ? "bg-red-100 text-orange-500"
                                                                    : "bg-orange-100 text-orange-500"
                                                                    }`}
                                                            >
                                                                {canRectify ? "Rectification Pending" : "Rectifying"}
                                                            </span>
                                ...
                                <div className="mb-6">
                                    <div className="mb-2 flex items-center gap-2">
                                        ...
                                        <h2 className="text-sm font-semibold text-foreground sm:text-base text-green-500">Completed Checklists</h2>
