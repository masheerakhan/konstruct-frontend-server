...
export const DynamicSafetyInput = ({
  item,
  value,
  secondaryValue,
  onChange,
  ...
  const secondaryOptions =
    Array.isArray(item.secondary_options) && item.secondary_options.length
      ? item.secondary_options
      : [];
...
// MakerDashboard
// ─────────────────────────────────────────────
export default function SafetyChecklistCreateView({
  onBack,
  allowedFirstRoles = ["MAKER", "INITIALIZER"],
}) {
    ...
    try {
      ...
      const activeTemplates = list.filter((t) => {
        ...
        const firstRole = firstWorkflowStep
          ? String(firstWorkflowStep.role).toUpperCase().trim()
          : "";
  ...
  const makerCanRectify = (c) => makerInWorkflow(c) && isMakerTurn(c);
  ...
  const isCreatorFirstStep =
    initialCreateWorkflowStep &&
    allowedFirstRoles.includes(
      String(initialCreateWorkflowStep.role).toUpperCase().trim(),
    );
  const createAssignedQuestionsIndices = isCreatorFirstStep
  ...
  const handleSubmitAllFixes = async () => {
    ...
    makerFixItems
      ...
      .forEach(({ sub, item, isMock }) => {
        ...
        const answer =
          typeof ansData === "object" ? ansData?.answer || "" : ansData || "";
        const secondaryAnswer =
          typeof ansData === "object" ? ansData?.secondary_answer || "" : "";
    ...
    const hasMissingAnswer =
      ...
      Object.values(answers).some(
        (a) =>
          a.type !== "file_upload" && (typeof a === "object" ? !a.answer : !a),
      );
  ...
  const handleCreateAndSubmitChecklist = async () => {
    ...
    const unansweredIndex = assignedQuestionsToAnswer.findIndex((q) => {
      const type = q.input_type || q.type || "multiple_choice";
      if (type === "file_upload") return false;
      const ansObj = createAnswers[q.id];
      const ansText =
        typeof ansObj === "object" && ansObj !== null ? ansObj.answer : ansObj;
                ...
                {!templateLoading && selectedTemplate && (
                  <>
                    <div className="mb-4 rounded-xl border bg-card p-5 shadow-sm">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {
                          (selectedTemplate.questions || []).filter((q, idx) =>
                            isCreateAssigned(idx),
                          ).length
                        }{" "}
                        question(s)
                    ...
                    {!isCreatorFirstStep && (
                      <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
                        The first step of this checklist workflow is assigned to
                        another role. Please complete the Inspection Details
                        above and click "Submit Checklist" to initiate the
                        process.
                      </div>
                    )}
                          ...
                          >
                            ...
                            <div className="space-y-3">
                              <DynamicSafetyInput
                                item={q}
                                value={
                                  createAnswers[q.id]?.answer ??
                                  (typeof createAnswers[q.id] === "string"
                                    ? createAnswers[q.id]
                                    : "")
                                }
                                secondaryValue={
                                  createAnswers[q.id]?.secondary_answer ?? ""
                                }
                                onChange={(value) =>
                            ...
                            <div className="space-y-3">
                              ...
                              {editable ? (
                                <DynamicSafetyInput
                                  item={item}
                                  value={
                                    makerAnswers[sub.id]?.answer ??
                                    (typeof makerAnswers[sub.id] === "string"
                                      ? makerAnswers[sub.id]
                                      : "")
                                  }
                                  secondaryValue={
                                    makerAnswers[sub.id]?.secondary_answer ?? ""
                                  }
                                  onChange={(value) => {
                                ...
                                <div className="flex flex-col gap-1.5">
                                  <RemarksBubble
                                    ...
                                    text={
                                      sub.answer ||
                                      (typeof makerAnswers[sub.id] === "object"
                                        ? makerAnswers[sub.id]?.answer
                                        : makerAnswers[sub.id]) ||
                                      "No Answer"
                                    }
                                  />
                                  {(sub.secondary_answer ||
                                    (typeof makerAnswers[sub.id] === "object"
                                      ? makerAnswers[sub.id]?.secondary_answer
                                      : null)) && (
                                    <RemarksBubble
              ...
              <section>
                ...
                <div className="mb-6">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-semibold text-foreground sm:text-base text-green-500">
