...
const AnswerPill = ({ answer }) => {
    ...
    return (
        <span className={`inline-flex items-center rounded-full px-3 py-0.5 text-xs font-bold ${cls}`}>
            {answer}
        </span>
    );
};
...
const ThreadEntry = ({ entry }) => {
    const { role, attemptNo, actorName, action, answer, comment, photoUrl, date } = entry;
    ...
    return (
        <div className={`flex gap-2.5 ${isMaker ? "flex-row" : "flex-row-reverse"}`}>
            {/* Avatar */}
            <div
                className={`mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full shadow-sm ${isMaker
            ...
            {/* Content */}
            <div className={`flex min-w-0 max-w-[80%] flex-col ${isMaker ? "items-start" : "items-end"}`}>

                {/* Meta row */}
                <div className={`mb-1.5 flex flex-wrap items-center gap-1.5 ${isMaker ? "" : "flex-row-reverse"}`}>
                    <span className={`text-xs font-semibold ${isMaker ? "text-orange-700" : "text-blue-700"}`}>
                ...
                {/* Bubble */}
                <div
                    className={`w-full rounded-2xl border px-4 py-3 shadow-sm ${isMaker
...
function SafetyChecklistHistoryModal({ open, onClose, checklistId, title }) {
    ...
    const approvedQuestionsCount = questions.filter(([itemId]) =>
        getQuestionFinalStatus(attempts, itemId) === "approved"
    ).length;

    const rejectedQuestionsCount = questions.filter(([itemId]) =>
        getQuestionFinalStatus(attempts, itemId) === "rejected"
    ).length;
                ...
                <div className="flex items-start justify-between gap-3 border-b px-6 py-4">
                    <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {overallStatusLabel && (
                                <StatusPill status={overallStatusLabel} />
                            )}
                        </div>
                        ...
                        {!loading && questions.length > 0 ? (
                            <>
                                ...
                                <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1 font-semibold text-green-700">
                                    <CheckCircle className="h-3.5 w-3.5" />
                                    {approvedQuestionsCount} Approved question{approvedQuestionsCount !== 1 ? "s" : ""}
                                </span>

                                <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-3 py-1 font-semibold text-red-700">
                                    <AlertTriangle className="h-3.5 w-3.5" />
                                    {rejectedQuestionsCount} Rejected question{rejectedQuestionsCount !== 1 ? "s" : ""}
                                </span>
                            </>
                        ) : !loading ? (
                            "No history found"
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5">
                    ...
                    {!loading && !history?.summary?.history_available && (
                        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                            {history?.summary?.message || "No history found for this checklist."}
                        </div>
                    )}
