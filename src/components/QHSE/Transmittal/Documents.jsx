...
import QualitySignages from "../Signages/Quality/QualitySignages";
import HSESignages from "../Signages/HSE/HSESignages";
import { takePendingAdd } from "./transmittalStorage";
import InspectionChecklistManager from "../InspectionChecklists/InspectionChecklistManager";
...
import WorkInspectionRequest from "../WIRandMOR/WIR/WorkInspectionRequest";
import WIRRegister from "../WIRandMOR/WIR/WIRRegister";
import AreaClearanceForm from "../WIRandMOR/WIR/AreaClearanceForm";
...
import ExternalSORBulkRegister from "../SOR/ExternalSORBulkRegister";
...
import PaymentHoldAbeyanceNote from "../DebitNote/PaymentHoldAbeyanceNote";
...
import LegalDocumentForm from "../LegalDocuments/LegalDocumentForm";
...
import IncidentReport from "../IncidentAccidentLTIRecords/IncidentReport/IncidentReport";
import IncidentRegister from "../IncidentAccidentLTIRecords/IncidentReport/IncidentRegister";
...
import TransmittalRegister from "../DMS/TransmittalRegister";
import MOR from "../WIRandMOR/MOR/MockupInspectionReport";

// impoer HSEMonthlyReport
import AuditChecklistRmcPlant from "../ReadyMixConcrete/AuditChecklistRmcPlant";

import SnagListChecklist from "../ProjectCloseout/SnagList/SnagListChecklist";

import HandingOverDocumentChecklist from "../ProjectCloseout/HandingOverDocumentChecklist";

import VirtualCompletionCertificate from "../ProjectCloseout/VirtualCompletion/VirtualCompletionCertificate";

import CompletionHandoverCertificate from "../ProjectCloseout/CompletionHandoverCertificate";
import WarrantyCertificate from "../ProjectCloseout/WarrantyCertificate";

const RESOURCES_ROOT_NAME = "Resources";
const RESOURCES_DEFAULT_SUBFOLDERS = ["Minimum Manpower", "QC Assets"];
const CONSTRUCTION_PROGRAMS_ROOT_NAMES = [
  "Construction Programs",
  "Construction Programmes",
];
const CONSTRUCTION_PROGRAMS_ROOT_ALIASES = [
  ...CONSTRUCTION_PROGRAMS_ROOT_NAMES,
  "Daily Labour & Progress Report & MOM",
  "Daily Labour & Progress Report and MOM",
  "Daily Labour & Progress Report",
  "Daily Progress & Labour Report",
];
...
function mapMinimumManpowerRowFromApi(r) {
  ...
  return {
    ...
    actual_deployment:
      r.actual_deployment != null ? String(r.actual_deployment) : "",
  };
}
...
async function ensureDefaultConstructionProgramsFolders(
  handleApiError,
  dmsScope = {},
) {
  if (!dmsScope.org_id || !dmsScope.project_id) return;

  if (constructionProgramsFolderEnsurePromise)
...
function includesAnyFolderName(value, candidates) {
  ...
  return candidates.some(
    (candidate) => normalized === normalizeDmsFolderTitle(candidate),
  );
}
...
function getTransmittalDocTypeByFolder(folder) {
  ...
  if (
    includesAnyFolderName(name, [
      ...
      // "Method Statements",
      // "WMS",
      "Work Method Statements (WMS,ITP Checklist & JSA)",
      "work method statements (wms,itp checklist & jsa)",
    ])
  ) {
...
function isTransmittalSubmissionFolderNode(folder) {
  if (!folder) return false;

  return includesAnyFolderName(folder.name, [
    "Transmittal Submission",
    "Transmittal Submissions",
    "Manual Transmittal Submission",
    "Manual Transmittal",
  ]);
}
...
function isRegistersBoxFilesFolderNode(folder) {
  if (!folder) return false;
  const kind = String(folder.folderKind || "")
    .trim()
    .toLowerCase();
    ...
    >
      <td className="pl-4 pr-2 py-3 text-gray-500">
        <div className="flex items-center gap-1.5">
          ...
          <span className="leading-none pt-[1px] pl-4">
            {file.sequenceNo ?? file.sequence_no ?? index + 1}
          </span>
        </div>
      </td>
      ...
      <td className="px-4 py-3 text-gray-500">
        <div className="flex items-center gap-1">
          ...
          <span className="whitespace-nowrap">
            {formatDisplayDateTime(
              file.uploaded_at || file.created_at || file.createdAt,
            )}
          </span>
        </div>
      </td>
...
function DocumentManager() {
  ...
  const [folderAbbreviation, setFolderAbbreviation] = useState("");
  const [searchQuery, setSearchQuery] = useState("");
  const [sortConfig, setSortConfig] = useState({ key: null, direction: "asc" });
  const [rootViewMode, setRootViewMode] = useState("list");
  const [subfolderViewMode, setSubfolderViewMode] = useState("list");
  ...
  const SortHeader = ({ label, sortKey, className = "" }) => (
    ...
    >
      <div className="flex items-center gap-1 whitespace-nowrap">
        <ArrowUpDown
          className={`w-3 h-3 shrink-0 ${sortConfig.key === sortKey ? "text-primary" : "text-gray-400"}`}
        />
        {label}
      </div>
    </th>
      ...
      switch (sortConfig.key) {
        ...
        case "date":
          if (type === "doc") {
            ...
            aVal = new Date(
              a.uploaded_at || a.created_at || a.createdAt || 0,
            ).getTime();
            bVal = new Date(
              b.uploaded_at || b.created_at || b.createdAt || 0,
            ).getTime();
          } else {
            aVal = new Date(a.created_at || a.createdAt || 0).getTime();
            bVal = new Date(b.created_at || b.createdAt || 0).getTime();
  ...
  const [folderGreenfieldSequenceNo, setFolderGreenfieldSequenceNo] =
    useState(0);
  const [folderBrownfieldSequenceNo, setFolderBrownfieldSequenceNo] =
    useState(0);
  ...
  const [activeDmsProjectCategory, setActiveDmsProjectCategory] = useState(
    cleanStorageValue(localStorage.getItem("ACTIVE_PROJECT_CATEGORY")) ||
      "greenfield",
  );
  ...
  useEffect(() => {
    ...
    async function loadDmsProjects() {
      try {
        ...
        const options = rawProjects.map(normalizeProjectOption).filter(Boolean);

        if (cancelled) return;

        setDmsProjects(options);

        const storedProjectId = cleanStorageValue(
          localStorage.getItem("ACTIVE_PROJECT_ID"),
        );
        const storedProject = options.find(
          (p) => String(p.id) === String(storedProjectId),
        );

        // If stored project is valid, keep it
        if (storedProject) {
          setActiveDmsProjectId(storedProject.id);
          setActiveDmsProjectCategory(
            storedProject.projectCategory || "greenfield",
          );
  ...
  const normalizeFolderNode = useCallback(
    (node) => ({
      id: node.id,
      orgId: node.org_id != null ? String(node.org_id) : (node.orgId ?? ""),
      projectId:
      ...
      greenfieldSequenceNo:
        node.greenfield_sequence_no != null &&
        Number.isFinite(Number(node.greenfield_sequence_no))
          ? Number(node.greenfield_sequence_no)
          : null,

      brownfieldSequenceNo:
        node.brownfield_sequence_no != null &&
        Number.isFinite(Number(node.brownfield_sequence_no))
          ? Number(node.brownfield_sequence_no)
          : null,

      effectiveSequenceNo:
        node.effective_sequence_no != null &&
        Number.isFinite(Number(node.effective_sequence_no))
          ? Number(node.effective_sequence_no)
          : node.sequence_no != null &&
              Number.isFinite(Number(node.sequence_no))
            ? Number(node.sequence_no)
            : 0,

      folderKind:
        node.folder_kind != null
          ? String(node.folder_kind)
          : (node.folderKind ?? ""),
      projectCategory:
      ...
      createdBy:
        node.created_by != null
          ? String(node.created_by)
          : node.createdBy != null
            ? String(node.createdBy)
            : "",
      size:
      ...
      itemCount:
        node.item_count != null && Number.isFinite(Number(node.item_count))
          ? Number(node.item_count)
          : null,
  ...
  const loadFolderDetailData = useCallback(
    async (fid) => {
      ...
      // Guard: don't fetch until dmsScope is fully resolved (prevents blank page on refresh)
  ...
  const handleDragEnd = async (event) => {
    ...
    setFolderMap((prevMap) => {
      ...
      const oldIndex = current.files.findIndex(
        (f) => String(f.id) === String(active.id),
      );
      const newIndex = current.files.findIndex(
    ...
    setCurrentFolderDetail((prevDetail) => {
      ...
      return {
        ...prevDetail,
        files: updatedFilesList.length ? updatedFilesList : prevDetail.files,
      };
    });
    ...
    if (updatedFilesList.length > 0) {
      patchDmsFileReorder(
        folderId,
        updatedFilesList.map((f) => ({ id: f.id, sequence_no: f.sequenceNo })),
      )
  ...
  const openEditFolderModal = useCallback((folder) => {
    ...
    setFolderAbbreviation(String(folder.abbreviation || ""));
    setFolderSequenceNo(
      Number.isFinite(Number(folder.sequenceNo))
        ? Math.max(0, Number(folder.sequenceNo))
        : 0,
    );
  ...
  const handleSubmitFolder = async () => {
    ...
    try {
      ...
      if (!dmsScope.org_id || !dmsScope.project_id) {
        toast.error(
          "Please select an active project before creating a DMS folder.",
        );
        return;
      }
      ...
      const folderPayload = {
        ...
        abbreviation: folderAbbreviation.trim(),
        sequence_no: baseSequenceNo,
        greenfield_sequence_no:
          folderProjectCategory === "greenfield" ||
          folderProjectCategory === "both"
      ...
      handleApiError(err, editingFolderId ? "update folder" : "create folder", {
        toast: true,
      });
    }
  };
  ...
  const openFolder = (folderOrId) => {
    ...
    if (isTransmittalSubmissionFolderNode(folder)) {
      navigate(
        createDocumentPath(null, targetId, "", getCurrentDocumentsPath()),
      );
      return;
    }
  ...
  const isInResourcesTree = (folder) => {
    ...
    while (cur) {
      if (
        String(cur.name).toLowerCase() ===
        String(RESOURCES_ROOT_NAME).toLowerCase()
      )
        return true;
  ...
  const { specialResourcesLeaf, resourceLeafKind } = useMemo(() => {
    ...
    const parent =
      currentFolder.parentId != null
        ? map.get(String(currentFolder.parentId))
        : null;
    const underResources =
      parent &&
      String(parent.name || "").toLowerCase() ===
        RESOURCES_ROOT_NAME.toLowerCase();
    return {
  ...
  const isMinimumManpowerFolder =
    specialResourcesLeaf && resourceLeafKind === "mm";

  const isQcAssetsFolder = specialResourcesLeaf && resourceLeafKind === "qc";
  const isEscalationMatrixFolder = useMemo(() => {
    if (!currentFolder) return false;
    const byKind = String(currentFolder.folderKind || "")
      .trim()
      .toLowerCase();
  ...
  const isCommunicationMatrixFolder = useMemo(() => {
    ...
    return (
      normalized === "communication matrix" ||
      normalized === "vendor onboarding process"
    );
  }, [currentFolder, isEscalationMatrixFolder]);

  const isMomMinutesFolder = useMemo(() => {
    if (!currentFolder || !isLikelyMomFolderName(currentFolder.name))
      return false;
    const map = new Map(Object.values(folderMap).map((f) => [String(f.id), f]));
  ...
  const isQualitySignagesFolder = useMemo(() => {
    if (!currentFolder) return false;
    const normalized = String(currentFolder.name || "")
      .toLowerCase()
      .replace(/\s+/g, " ")
      .trim();
    return normalized === "quality signages";
  }, [currentFolder]);

  const isHSESignagesFolder = useMemo(() => {
    if (!currentFolder) return false;
    const normalized = String(currentFolder.name || "")
      .toLowerCase()
      .replace(/\s+/g, " ")
      .trim();
    return normalized === "hse signages";
  }, [currentFolder]);
  ...
  const isMaterialInwardRegisterFolder = useMemo(() => {
    ...
    return (
      parentName.includes("material inspection") || parentName.includes("mir")
    );
  }, [currentFolder, folderMap]);
  ...
  const isProjectPlanRegisterFolder =
    currentTransmittalDocType === "Project Plans";
  const isMaterialSubmissionRegisterFolder =
    currentTransmittalDocType === "Material Submittal";
  ...
  const isTransmittalRegister = useMemo(() => {
    if (!currentFolder) return false;
  ...
  const isExternalObservationMechanismBulkFolder = useMemo(() => {
    if (!currentFolder) return false;
    return includesAnyFolderName(currentFolder.name, [
      "external sor (bulk)",
      "External SOR (Bulk)",
  ...
  const isMORFolder = useMemo(() => {
    if (!currentFolder) return false;

    return includesAnyFolderName(currentFolder.name, [
      "Mock-up Inspection Request",
      "mock-up inspection request",
    ]);
  });
  ...
  const isPaymentHoldAbeyanceNoteFolder = useMemo(() => {
    if (!currentFolder) return false;

    const curName = normalizeDmsFolderTitle(currentFolder.name);

    if (
      curName !== "payment hold abeyance note" &&
      curName !== "payment hold abeyance" &&
      curName !== "payment hold"
    ) {
      return false;
    }
  ...
  const isLegalDocumentFolder = useMemo(() => {
    if (!currentFolder) return false;
    const curName = normalizeDmsFolderTitle(currentFolder.name);
    return (
      curName === "legal document" ||
      curName === "legal documents" ||
      curName.includes("legal documents - hse") ||
      curName.includes("legal document - hse")
    );
  }, [currentFolder]);

  const isIncidentReportFolder = useMemo(() => {
    if (!currentFolder) return false;
  ...
  const isAuditChecklistRmcPlantFolder = useMemo(() => {
    if (!currentFolder) return false;

    return includesAnyFolderName(currentFolder.name, [
      "Audit Checklist - RMC Plant",
      "Audit Checklist RMC Plant",
      "RMC Plant Audit Checklist",
  ...
  const isInspectionChecklistFolder = useMemo(() => {
    if (!currentFolder) return false;

    return includesAnyFolderName(currentFolder.name, [
      "Inspection Checklists & 3rd Party Certificates",
      "Inspection Checklists",
    ]);
  }, [currentFolder]);

  const isSnagListFolder = useMemo(() => {
    if (!currentFolder) return false;
  ...
  const isHandingOverDocumentChecklistFolder = useMemo(() => {
    if (!currentFolder) return false;

    return includesAnyFolderName(currentFolder.name, [
      "Handing Over Document Checklist",
      "Handing Over Documents Checklist",
      "Handing Over Document Checklist (HOD)",
      "Handover Check sheet",
      "Handover Checklist",
      "HOD",
    ]);
  }, [currentFolder]);

  const isVirtualCompletionCertificateFolder = useMemo(() => {
    if (!currentFolder) return false;
  ...
  const isCompletionHandoverCertificateFolder = useMemo(() => {
    if (!currentFolder) return false;
  ...
  const isWarrantyCertificateFolder = useMemo(() => {
    if (!currentFolder) return false;
  ...
  const isSpecialCreatedFolder =
    ...
    isPaymentHoldAbeyanceNoteFolder ||
    isSafetyCommitteeMeetingAgendaFolder ||
    isMockDrillObservationReportFolder ||
    isLegalDocumentFolder ||
    isIncidentReportFolder ||
    ...
    isAuditChecklistRmcPlantFolder ||
    isSnagListFolder ||
    isHandingOverDocumentChecklistFolder ||
    isVirtualCompletionCertificateFolder ||
    isCompletionHandoverCertificateFolder ||
    isWarrantyCertificateFolder ||
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled)
          handleApiError(err, "load projects for resource registers");
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          const raw = Array.isArray(res?.data?.minimum_manpower)
            ? res.data.minimum_manpower
            : [];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          const raw = Array.isArray(res?.data?.qc_assets)
            ? res.data.qc_assets
            : [];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setEscalationRows([]);
      try {
        const res = await getDmsEscalationMatrix({
          project_id: String(cmProjectId),
        });
        const rows = Array.isArray(res?.data?.rows) ? res.data.rows : [];
        const pname =
          res?.data?.project_name != null ? String(res.data.project_name) : "";
        if (!cancelled) {
  ...
  const handleCreateDocument = () => {
    const docTypeForFolder = isTransmittalSubmissionFolderNode(currentFolder)
      ? ""
      : getPreferredFolderDocType(currentFolder);
  ...
  const handlePrimaryCreateAction = () => {
    if (specialResourcesLeaf && currentFolder) {
      ...
      if (n === "minimum manpower" || n === "qc assets" || n === "qc asset")
        return;
      return;
    }

    if (isTransmittalSubmissionFolderNode(currentFolder)) {
      navigate(
        createDocumentPath(
          null,
          currentFolder.id,
  ...
  const handleOpenSubfolder = async (folderName) => {
    ...
    const existing = childFolders.find(
      (f) =>
        normalizeDmsFolderTitle(f.name) === normalizeDmsFolderTitle(folderName),
    );
    if (existing) {
    ...
    try {
      const folderProjectCategory = String(
        currentFolder.projectCategory ||
          activeDmsProjectCategory ||
          "greenfield",
      ).toLowerCase();
      const maxSeq = childFolders.reduce((max, f) => {
        return Math.max(
          ...
          Number(
            f.sequenceNo ||
              f.greenfieldSequenceNo ||
              f.brownfieldSequenceNo ||
              0,
          ),
        );
      }, 0);
  ...
  return (
    <div className={specialChildFormOpen ? "" : "p-6"}>
      {!specialChildFormOpen && (
        <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
              ...
              <div className="mt-3 flex items-center gap-2 flex-wrap">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const selected = dmsProjects.find(
                      (p) => String(p.id) === String(projectId),
                    );

                    setActiveDmsProjectId(projectId);
                    setActiveDmsProjectCategory(
                      selected?.projectCategory || "greenfield",
                    );
                ...
                >
                  ...
                  {dmsProjects.map((project) => (
                    <option key={project.id} value={project.id}>
                      {project.name} —{" "}
                      {String(project.projectCategory || "greenfield")}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {!breadcrumbChain.some((f) =>
              ["format", "samples", "library"].includes(
                normalizeDmsFolderTitle(f.name).toLowerCase(),
              ),
            ) &&
              (currentFolder?.parentId || isSpecialCreatedFolder) && (
                <>
                  <button
                    type="button"
      ...
      {currentFolder ? (
        <div>
          ...
          {(!isSpecialCreatedFolder || specialFolderTab === "created") && (
            <>
              {isQualitySignagesFolder && <QualitySignages />}
              {isHSESignagesFolder && <HSESignages />}
              ...
              {isMinimumManpowerFolder && (
                <MinimumManpowerRegisterTable
                  ...
                  onAddRow={() =>
                    setManpowerRows((prev) => [...prev, emptyMmrCustomRow()])
                  }
                  onRemoveRow={(index) =>
                    ...
                    setManpowerRows((prev) =>
                      prev.map((r, i) => {
                        ...
                        if (field === "deployment") {
                          ...
                          return {
                            ...r,
                            deployment: ["B", "P", "S"].includes(v) ? v : "",
                          };
                        }
                        if (field === "actual_deployment") {
                          return {
                            ...r,
                            actual_deployment: String(value ?? "").slice(
                              0,
                              255,
                            ),
                          };
                        ...
                        if (field === "position") {
                          return {
                            ...r,
                            position: String(value ?? "").slice(0, 500),
                          };
                        }
                        if (field === "min_experience_years") {
                          return {
                            ...r,
                            min_experience_years: String(value ?? "").slice(
                              0,
                              128,
                            ),
                          };
                        ...
                        if (field === "required_number") {
                          return {
                            ...r,
                            required_number: String(value ?? "").slice(0, 128),
                          };
                        }
                    ...
                    try {
                      const res = await saveDmsMinimumManpowerRegister({
                        ...
                        minimum_manpower: manpowerRows.map((r) => {
                          const isCustom =
                            Boolean(r.is_custom) || r.template_id == null;
                          if (isCustom) {
                            return {
                              template_id: null,
                              is_custom: true,
                              ...
                              deployment: normalizeDmsMmrDeployment(
                                r.deployment,
                              ),
              ...
              {isQcAssetsFolder && (
                <QCAssetsRegisterTable
                  ...
                  onAddRow={() =>
                    setQcRows((prev) => [...prev, emptyQcCustomRow()])
                  }
                  onRemoveRow={(index) =>
                    setQcRows((prev) => prev.filter((_, i) => i !== index))
                  }
                  onCellChange={(index, field, value) => {
                    setQcRows((prev) =>
                      prev.map((r, i) => {
                        ...
                        if (field === "description") {
                          return {
                            ...r,
                            description: String(value ?? "").slice(0, 255),
                          };
                        }
                        if (field === "minimum_number") {
                          return {
                            ...r,
                            minimum_number: String(value ?? "").slice(0, 128),
                          };
                        }
                        return { ...r, [field]: value };
                      }),
                      ...
                      const res = await saveDmsQcAssetsRegister({
                        ...
                        qc_assets: qcRows.map((r) => {
                          const isCustom =
                            Boolean(r.is_custom) || r.template_id == null;
                          const ac =
                            r.actual_count === "" || r.actual_count == null
                              ? null
                              : Number(r.actual_count);
                          if (isCustom) {
                            return {
                              template_id: null,
                              is_custom: true,
              ...
              {isMomMinutesFolder && (
                <MomFolderView folderId={currentFolder.id} />
              )}

              {isRegistersBoxFilesFolder && (
                <BoxFileRegister folderId={currentFolder.id} />
              )}

              {/* {isMirFolder && <MIRForm />} */}

              {isMirFolder && (
                <MIRRegister onFormOpenChange={setSpecialChildFormOpen} />
              )}
              ...
              {isWirWorkInspectionRequestFolder && (
                <WIRRegister onFormOpenChange={setSpecialChildFormOpen} />
              )}
              ...
              {isExternalObservationMechanismBulkFolder && (
                <ExternalSORBulkRegister
                  onFormOpenChange={setSpecialChildFormOpen}
                />
              )}
              {isPaymentHoldAbeyanceNoteFolder && (
                <div className="mt-4">
                  <PaymentHoldAbeyanceNote rootFolders={dedupedRootFolders} />
                </div>
              )}

              {isDebitNoteFolder && (
                <DebitNoteRegister onFormOpenChange={setSpecialChildFormOpen} />
              )}
              ...
              {isLegalDocumentFolder && (
                <LegalDocumentForm
                  ...
                  onBack={() => {
                    setSpecialChildFormOpen(false);
                    if (currentFolder?.parentId) {
                      setSearchParams({ folder: currentFolder.parentId });
                    } else {
                      setSearchParams({});
                    }
                  }}
              ...
              {isIncidentReportFolder && (
                <IncidentRegister onFormOpenChange={setSpecialChildFormOpen} />
              )}
              ...
              {isTrainingQualityRegisterFolder && <TrainingQualityRegister />}

              {isTrainingHSERegisterFolder && <TrainingHSERegister />}
              ...
              {isAuditChecklistRmcPlantFolder && (
                <AuditChecklistRmcPlant
                  folderId={currentFolder.id}
                  folderName={currentFolder.name}
                />
              )}

              {isInspectionChecklistFolder && (
                <InspectionChecklistManager
                  folderId={currentFolder.id}
                  projectId={activeDmsProjectId}
                />
              )}
          ...
          {(() => {
            ...
            if (isQualitySignagesFolder || isHSESignagesFolder) {
              return null;
            }
            ...
            if (isInspectionChecklistFolder) {
              return null;
            }
            ...
            const folders = applySort(
              allFolders.filter((f) =>
                (f.name || "").toLowerCase().includes(lowerQuery),
              ),
              "folder",
            );
            const docs = applySort(
              allDocs.filter(
                (d) =>
                  (d.refNo || "").toLowerCase().includes(lowerQuery) ||
                  (d.name || "").toLowerCase().includes(lowerQuery),
              ),
              "doc",
            );
            const files = applySort(
              allFiles.filter((f) =>
                (f.name || "").toLowerCase().includes(lowerQuery),
              ),
              "file",
            ...
            if (!hasItems && !showUpload && !isInResourcesTree(currentFolder)) {
              return (
                <p className="text-sm text-gray-500">
                  ...
                  {capitalCase("use create transmittal to add a document")}.
                </p>
              );
            }
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between w-full">
                  ...
                  <div className="flex items-center bg-gray-100 rounded-lg p-1">
                    <button
                      onClick={() => setSubfolderViewMode("list")}
                      className={`p-1.5 rounded-md transition-colors ${subfolderViewMode === "list" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                      title="List View"
                    >
                      <List size={18} />
                    </button>
                    <button
                      onClick={() => setSubfolderViewMode("grid")}
                      className={`p-1.5 rounded-md transition-colors ${subfolderViewMode === "grid" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                      title="Grid View"
                    >
                      <Grid size={18} />
                    </button>
                ...
                <div
                  className={`${subfolderViewMode === "list" ? "bg-white rounded-lg border border-gray-200 overflow-hidden" : ""}`}
                >
                  {(showUpload || files.length > 0) && (
                    <div
                      className={`px-4 py-3 flex items-center justify-between ${subfolderViewMode === "list" ? "border-b border-gray-100" : "mb-4 bg-white rounded-lg border border-gray-200"}`}
                    >
                      <div>
                        {showUpload && (
                          <>
                            <input
                              ...
                              multiple
                              accept={
                                isSpecialCreatedFolder &&
                                specialFolderTab === "documents"
                  ...
                  {subfolderViewMode === "grid" ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-4">
                      {folders.map((folder) => (
                        <div
                          key={`folder-${folder.id}`}
                          className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer flex flex-col justify-between group"
                          onClick={() => openFolder(folder)}
                        >
                          <div>
                            <div className="flex items-start justify-between mb-2">
                              <div className="flex items-start gap-3 w-full">
                                ...
                                <div className="min-w-0 flex-1">
                                  <h3
                                    className="text-base font-semibold text-gray-800 line-clamp-2 mb-1"
                              ...
                              {canEditFolders && (
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-2">
                                  <button
                                    ...
                                    className="p-1.5 rounded-md text-gray-500 hover:bg-orange-50 hover:text-primary transition-colors"
                                  >
                            ...
                            <div className="flex items-start justify-between mb-2">
                              <div className="flex items-start gap-3 w-full">
                                ...
                                <div className="min-w-0 flex-1">
                                  <h3
                                    className="text-base font-semibold text-gray-800 line-clamp-2 mb-1"
                                    title={doc.refNo || doc.name}
                                  >
                                    {doc.refNo || doc.name || "Document"}
                                  </h3>
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center justify-end text-xs text-gray-400 pt-2">
                      ...
                      {files.map((file) => (
                        <div
                          key={`file-${file.id}`}
                          className={`bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer flex flex-col justify-between relative ${selectedFiles.has(file.id) ? "ring-2 ring-primary" : ""}`}
                          onClick={() => handleOpenFile(currentFolder.id, file)}
                        >
                          {isSelectionMode && (
                          ...
                          <div>
                            <div className="flex items-start justify-between mb-2">
                              ...
                              <div className="flex gap-1 z-10 relative shrink-0 ml-2">
                                <button
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleDownloadFile(currentFolder.id, file);
                                  }}
                                  className="p-1.5 rounded-md text-gray-500 hover:bg-primary/10 hover:text-primary transition-colors"
                                >
                                  <Download className="h-4 w-4" />
                                </button>
                                {canEditFolders && (
                            ...
                            <div className="flex items-center gap-1.5">
                              {files.length > 0 && isSelectionMode && (
                                <input
                                  ...
                                  onChange={() =>
                                    handleSelectAll(files.map((f) => f.id))
                                  }
                                />
                              )}
                              {files.length > 0 && canEditFolders && (
                                <div className="w-[19px] shrink-0" />
                              )}
                              <div
                                onClick={() => handleSort("sequenceNo")}
                                className="cursor-pointer hover:bg-gray-100/50 flex items-center gap-1 whitespace-nowrap"
                              >
                                <ArrowUpDown
                                  className={`w-3 h-3 shrink-0 ${sortConfig.key === "sequenceNo" ? "text-primary" : "text-gray-400"}`}
                                />
                                Sr. No.
                              </div>
                            </div>
                          ...
                          >
                            {files.map((file, index) => (
                              <SortableFileRow
                                ...
                                handleOpenFile={() =>
                                  handleOpenFile(currentFolder.id, file)
                                }
        ...
        <>
            ...
            <div className="mb-8">
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50/80">
                      <th className="text-left px-4 py-3 font-medium text-gray-600 w-[45%]">
                        Name
                      </th>
                      ...
                      <th className="text-left px-4 py-3 font-medium text-gray-600 w-[15%]">
                        Action
                      </th>
                    </tr>
                  </thead>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-mono text-gray-800 truncate">
                              {draft.formData?.transmittalRefNo ||
                                capitalCase("draft")}
                            </span>
                          </div>
                        </td>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50/80">
                      ...
                      <th className="text-left px-4 py-3 font-medium text-gray-600 w-[30%]">
                        Project
                      </th>
                      <th className="text-left px-4 py-3 font-medium text-gray-600 w-[20%]">
                        Date
                    ...
                    {unfiled.map((doc) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-gray-600 truncate">
                          {safeCapitalCaseName(doc.projectName)}
                        </td>
          ...
          {!currentFolder && (
            <div className="flex flex-col gap-4 mt-6">
              <div className="flex items-center justify-between w-full">
                ...
                <div className="flex items-center bg-gray-100 rounded-lg p-1">
                  <button
                    ...
                    className={`p-1.5 rounded-md transition-colors ${rootViewMode === "list" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                    title="List View"
                  >
                    <List size={18} />
                  </button>
                  <button
                    onClick={() => setRootViewMode("grid")}
                    className={`p-1.5 rounded-md transition-colors ${rootViewMode === "grid" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                    title="Grid View"
                  >
                    <Grid size={18} />
                  </button>
              ...
              <div
                className={`${rootViewMode === "list" ? "bg-white rounded-lg border border-gray-200 overflow-hidden" : ""}`}
              >
                {rootViewMode === "list" && (
                ...
                {(() => {
                  const lowerQuery = searchQuery.toLowerCase();
                  const filteredRoots = applySort(
                    dedupedRootFolders.filter((f) =>
                      (f.name || "").toLowerCase().includes(lowerQuery),
                    ),
                    "folder",
                  );
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
          ...
          >
            ...
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Folder Abbreviation (Optional)
            </label>
            <input
              type="text"
              placeholder="e.g. Doc Mgt"
              value={folderAbbreviation}
              onChange={(e) => setFolderAbbreviation(e.target.value)}
              maxLength={20}
              className="w-full border border-gray-200 px-3 py-2 rounded mb-4 focus:outline-none focus:ring-2 focus:ring-primary"
            />
            ...
            >
              ...
              <option value="box_file_register">
                Registers &amp; box files
              </option>
            </select>
