...
export default function FileUpload({ value = [], onChange, accept = "image/*,application/pdf", multiple = true, testId = "file-upload" }) {
  ...
  const remove = (id) => onChange && onChange((value || []).filter((f) => f.id !== id));
    ...
    <div className="space-y-3" data-testid={testId}>
      <label className="block cursor-pointer">
        <div className="border-2 border-dashed border-slate-300 rounded-lg bg-slate-50 hover:bg-slate-100 hover:border-blue-400 transition-colors p-6 text-center">
          ...
          <div className="text-sm font-medium text-slate-700 mt-2">Click to upload or drag &amp; drop</div>
          <div className="text-xs text-slate-500 mt-1">JPG, PNG, WebP, PDF — up to 10MB</div>
        </div>
        <input
          type="file"
          accept={accept}
      ...
      {(value || []).length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {(value || []).map((f) => (
            <div key={f.id} className="relative group border border-slate-200 rounded-lg overflow-hidden bg-white">
              {f.kind === "image" ? (
              ...
              <div className="p-2 text-[11px] truncate text-slate-600">{f.filename}</div>
              <button
                type="button"
